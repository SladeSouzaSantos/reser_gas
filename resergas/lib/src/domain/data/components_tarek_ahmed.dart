import '../models/component.dart';

class ComponentsTarekAhmed{
  
  static Map<String, Component> componentMap = {
        
    'Methane': Component(
      name: "Methane", 
      molecularWeight: 16.042,
      pseudocriticalPressure: 673.1, 
      pseudocriticalTemperature: -116.50,
      criticalZFactor: 0.289,
    ),
    
    'Ethane': Component(
      name: "Ethane", 
      molecularWeight: 30.068,
      pseudocriticalPressure: 708.3,
      pseudocriticalTemperature: 90.42,
      criticalZFactor: 0.285,
    ),
    'Propane': Component(
      name: "Propane",
      molecularWeight: 44.094,
      pseudocriticalPressure: 617.4,
      pseudocriticalTemperature: 206.59,
      criticalZFactor: 0.276,
    ),
    'nButane': Component(
      name: "nButane",
      molecularWeight: 58.120,
      pseudocriticalPressure: 550.7,
      pseudocriticalTemperature: 305.95,
      criticalZFactor: 0.274,
    ),
    'iButane': Component(
      name: "iButane",
      molecularWeight: 58.120,
      pseudocriticalPressure: 529.1,
      pseudocriticalTemperature: 275.29,
      criticalZFactor: 0.283,
    ),
    'nPentane': Component(
      name: "nPentane",
      molecularWeight: 72.146,
      pseudocriticalPressure: 489.5,
      pseudocriticalTemperature: 386.25,
      criticalZFactor: 0.268,
    ),
    'iPentane': Component(
      name: "iPentane",
      molecularWeight: 72.146,
      pseudocriticalPressure: 483.0,
      pseudocriticalTemperature: 370.33,
      criticalZFactor: 0.268,
    ),
    'nHexane': Component(
      name: "nHexane",
      molecularWeight: 86.172,
      pseudocriticalPressure: 439.7,
      pseudocriticalTemperature: 454.83,
      criticalZFactor: 0.264,
    ),
    'nHeptane': Component(
      name: "nHeptane",
      molecularWeight: 100.198,
      pseudocriticalPressure: 396.9,
      pseudocriticalTemperature: 512.95,
      criticalZFactor: 0.259,
    ),
    'nOctane': Component(
      name: "nOctane",
      molecularWeight: 114.224,
      pseudocriticalPressure: 362.1,
      pseudocriticalTemperature: 565.53,
      criticalZFactor: 0.258,
    ),
    'nNonane': Component(
      name: "nNonane",
      molecularWeight: 128.250,
      pseudocriticalPressure: 345.0,
      pseudocriticalTemperature: 613.33,
      criticalZFactor: 0.256,
    ),
    'nDecane': Component(
      name: "nDecane",
      molecularWeight: 142.276,
      pseudocriticalPressure: 320.0,
      pseudocriticalTemperature: 655.33,
      criticalZFactor: 0.247,
    ),

    /*'Hydrogen': Component(
      name: "Hydrogen",
      molecularWeight: 2.016,
      pseudocriticalPressure: 188.0,
      pseudocriticalTemperature: -399.47,
      criticalZFactor: 0.315,
    ),*/
    'Nitrogen': Component(
      name: "Nitrogen",
      molecularWeight: 28.016,
      pseudocriticalPressure: 492.0,
      pseudocriticalTemperature: -232.47,
      criticalZFactor: 0.289,
    ),
    /*'Oxygen': Component(
      name: "Oxygen",
      molecularWeight: 32.0,
      pseudocriticalPressure: 730.0,
      pseudocriticalTemperature: -181.47,
      criticalZFactor: 0.287,
    ),*/
    'HydrogenSulfide': Component(
      name: "HydrogenSulfide",
      molecularWeight: 34.076,
      pseudocriticalPressure: 1306.0,
      pseudocriticalTemperature: 212.73,
      criticalZFactor: 0.284,
    ),
    'CarbonDioxide': Component(
      name: "CarbonDioxide",
      molecularWeight: 44.010,
      pseudocriticalPressure: 1073.0,
      pseudocriticalTemperature: 88.33,
      criticalZFactor: 0.277,
    ),
  };
  
}